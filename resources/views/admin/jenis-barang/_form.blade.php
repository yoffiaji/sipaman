...
<div class="grid gap-4 md:grid-cols-2">
    <div>
        ...
        <input name="nama_jenis" value="{{ old('nama_jenis', $jenisBarang->nama_jenis ?? '') }}" required class="mt-1 w-full rounded-lg border-slate-300" placeholder="Contoh: Makanan Ringan">
    ...
    <div>
        ...
        <input name="slug" value="{{ old('slug', $jenisBarang->slug ?? '') }}" class="mt-1 w-full rounded-lg border-slate-300" placeholder="Otomatis jika dikosongkan">
...
<div class="mt-4">
    ...
    <textarea name="deskripsi" rows="3" class="mt-1 w-full rounded-lg border-slate-300" placeholder="Jelaskan isi kategori ini agar admin mudah memilih">{{ old('deskripsi', $jenisBarang->deskripsi ?? '') }}</textarea>
...
<div class="mt-4">
    ...
    <textarea name="aliases" rows="8" class="mt-1 w-full rounded-lg border-slate-300" placeholder="Satu keyword per baris, contoh:&#10;keripik&#10;kripik&#10;snack">{{ $aliases }}</textarea>
    @error('aliases')<p class="mt-2 text-sm text-red-700">{{ $message }}</p>@enderror
    <p class="mt-1 text-xs text-slate-500">Sistem mencocokkan keyword ini dengan Kategori Pangan dan Jenis Pangan dari file Rekap PIRT.</p>
</div>
...
<label class="mt-4 inline-flex items-center gap-2 rounded-lg border border-slate-200 px-3 py-2 text-sm font-semibold text-slate-700">
    ...
    <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $jenisBarang->is_active ?? true)) class="rounded border-slate-300 text-slate-900">
